...
- **`truetype.go`** — scalable TrueType/OpenType support via `golang.org/x/image/font/opentype`. `LoadTrueTypeFromFile`/`LoadTrueTypeFromBytes` build a `Font` rasterized at a fixed point size. A `Font` is BDF-backed (`font != nil`) or face-backed (`face != nil`, `isTrueType()`); the text-drawing/measuring methods in `text.go` dispatch on that. TrueType renders anti-aliased and supports the etched/shadowed `TextStyle`s. Arbitrary-angle rotation (`DrawStringRotatedAngle`) works for both: BDF rotates per-pixel, while TrueType rasterizes horizontally to a scratch image and affine-transforms it onto the destination (`drawTrueTypeGlyphsRotated`).
